    ...
    QWidget(parent)
{
    ...
    qDebug() << "initIMFileReceiverWidget";
...
void IMFileReceiverWidget::connectToHost(const QHostAddress & address)
{
    ...
    qDebug() << "m_hostAddress: " << m_hostAddress;
    m_blockSize = 0;
    m_fileReceiver->abort();
    //qDebug() << "connectToHost1";
    m_fileReceiver->connectToHost(m_hostAddress, m_tcpPort);
   // qDebug() << "connectToHost2";
    m_time.start();
}
...
void IMFileReceiverWidget::readMessage()
{
    ...
    qDebug() << "IMFileReceiverWidget::readMessage";
    ...
    if(m_bytesReceived <= sizeof(qint64)*2)
    {
                ...
                && (m_fileNameSize == 0))
        {
            ...
            qDebug() << "read file total size and file name size";
        }
