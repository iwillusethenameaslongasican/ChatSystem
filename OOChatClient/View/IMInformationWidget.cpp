...
UserInformation IMInformationWidget::getInformation()
{
    ...
    m_userInf.m_sex = m_cbSex->currentText();
...
void IMInformationWidget::onClickPreButton()
{
    --m_headNum;
    if(m_headNum < 1)
        m_headNum = MAX_NUM;
...
// private:----------------------------------------
void IMInformationWidget::initIMInformationWidget()
{
    m_isReadOnly = true;
    m_isEdit = false;
    ...
    m_cbSex->addItem(tr("男"));
    m_cbSex->addItem(tr("女"));
    ...
    vLayoutEditMid->addWidget(m_cbSex);
