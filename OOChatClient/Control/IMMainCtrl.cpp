  ...
  :  QObject(parent),m_myID(myID)
{
  ...
  connect(m_tcpSocket, &IMTcpSocket::connected,
          this, &IMMainCtrl::requestGetFriendsInformation);
  connect(m_tcpSocket, &IMTcpSocket::readyRead,
          this, &IMMainCtrl::readMessage);
  connect(m_tcpSocket, &IMTcpSocket::disconnected,
          this, &IMMainCtrl::closeWindow);
}
...
Description: 发送聊天信息
*************************************************/
void IMMainCtrl::sendTalkMessage(TalkMessage &msg)
{
  msg.m_senderID = m_myID;
  msg.m_senderIP = IMTcpSocket::getIP();
...
Description: 修改好友备注
*************************************************/
void IMMainCtrl::changeFriendRemark(const QString &myID, const QString &friendID, const QString &remark)
{
  if(NULL == m_tcpSocket)
    {
      return;
...
void IMMainCtrl::requestSendTalkMessage()
{
    ...
    while (m_messageVector.size() > 0)
    {
        ...
        out.device()->seek(0);
        out << quint16(block.size() - sizeof(quint16));
        qDebug() << "requestSendTalkMessage(TALK)";
        m_tcpSocket->write(block);
...
void IMMainCtrl::readMessage()
{
  ...
  switch(type)
    ...
    case GET_ALL_FRIENDS_SUCCESS:
      {
        qDebug() << "GET_ALL_FRIENDS_SUCCESS";
        in >> len;
        m_friendsVec.clear();
        ...
        emit getFriendSuccess(m_friendsVec);
        break;
      }
    case NO_FLOCK:
      {
    ...
    case GET_ALL_FLOCKS_SUCCESS:
      {
        ...
        qDebug() << "flocks len: " << len;
        emit getFlockSuccess(m_flocksVec);
        break;
      }
    case NO_DISCUSSION:
      {
    ...
    case GET_ALL_DISCUSSIONS_SUCCESS:
      {
        ...
        emit getDiscussionSuccess(m_discussionsVec);
        break;
      }
    ...
    case GET_FRIEND_SUCCESS:
      {
        in >> m_friendInf;
        emit getFriendInformationSuccess(m_friendInf);
        break;
      }
    ...
    case TALK:
      {
        ...
        qDebug() << "emit getFriendTalkMessage(m_message)";
        break;
      }
    case CHANGE_STATUE:
      {
        in >> m_peerID >> m_peerStatus;
        emit getFriendChangeStatus(m_peerID, m_peerStatus);
        break;
      }
    case CHANGE_INFORMATION:
      {
        in >> m_friendSimInf;
        emit getFriendChangeInformation(m_friendSimInf);
        break;
      }
    ...
    case ADD_FRIEND:
      {
        in >> m_message >> m_friendInf;
        emit getFriendRequest(m_message, m_friendInf);
        break;
      }
    case ADD_FLOCK:
      {
    ...
    case LEAVE_FLOCK_MEMBER:
      {
        in >> m_id >> m_flockID >> m_creatorID;
        emit haveMemberLeaveFlock(m_id, m_flockID, m_creatorID);
        break;
      }
    ...
    case GET_HISTORY_MESSAGE_SUCCESS:
      {
        ...
        for(int i = 0; i < len; i ++)
          {
 //            qDebug() << "client: " << m_HistoryMessageVector[i];
            in >> m_HistoryMessageVector[i];
          }
        qDebug() << "history message len: " << len;
        if(m_id == m_myID)
          {
            emit getNetworkHistoryMessageSuccess(m_peerID, m_date,m_HistoryMessageVector);
          }
        break;
      }
