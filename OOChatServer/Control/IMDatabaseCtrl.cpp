...
bool IMDatabaseCtrl::createConnection()
{
    ...
    // 如果已经打开
    if (QSqlDatabase::contains("serverIMDB.db"))
    {
        //qDebug() << 111;
        *m_db = QSqlDatabase::database("serverIMDB.db");
    }
    else
    {// 如果没有，就添加
       // qDebug() << 222;
        *m_db = QSqlDatabase::addDatabase("QSQLITE", "serverIMDB.db");
        m_db->setDatabaseName("serverIMDB.db");
    }
                                                  ...
                                                  UserInformation &userInf)
{
   ...
   // qDebug() << logInf.m_userID << " " << logInf.m_password;
    query.exec();
    errorSQLOrder(query, "login-search");

    QSqlDatabase::database().commit();
   // qDebug() << query.value(0).toString() << " " << query.value(3).toString();
    if (!query.isActive())
    {
...
void IMDatabaseCtrl::errorSQLOrder(QSqlQuery query, QString mark)
{
    ...
    if(!query.isActive())
    {
        QString str = query.lastError().text() + "/n" + mark;
...
//    }
    ...
    while (query.next())
    {
       ...
       /* qDebug() << "searchHistoryMessageOneFriendOneDate: "
                 <<mes.m_senderID << " " << mes.m_receiverID
                << " " << mes.m_text << " " << mes.m_dateTime;*/
